                    ...
                    <tbody>
                        @foreach ($result as $key => $data)
                            ...
                            <tr>
                                ...
                                @if ($customer->c_type == 1)
                                    <td>{{ sprintf('%.2f', $data->item_sale_rate_whole_base * (100 / (100 + $data->item_gst_slab))) }}
                                    </td>
                                @elseif($customer->c_type == 2)
                                    <td>{{ sprintf('%.2f', $data->item_sale_rate_retail_base * (100 / (100 + $data->item_gst_slab))) }}
                                    </td>
                                @endif
                                @if ($customer->c_type == 1)
                                    <td>{{ sprintf('%.2f', $temp = $data->s_i_qty * $data->item_sale_rate_whole_base * (100 / (100 + $data->item_gst_slab))) }}
                                    </td>
                                @elseif($customer->c_type == 2)
                                    <td>{{ sprintf('%.2f', $temp = $data->s_i_qty * $data->item_sale_rate_retail_base * (100 / (100 + $data->item_gst_slab))) }}
                                    </td>
                                @endif
                            </tr>
                            @php
                        ...
                        @foreach ($newSelling as $item)
                            ...
                            <tr>
                                ...
                                <td>{{ sprintf('%.2f', $temp = $item->s_i_rate * (100 / (100 + $item->s_i_tax)) * $item->s_i_qty) }}
                                </td>
                            </tr>
                            @php
                                $total += $temp;
                        ...
                        @php
                            ...
                            $uniqueItemSlabs = $result
                                ->pluck('item_gst_slab')
                                ->unique()
                                ->toArray();
                            $uniqueNewItemSlabs = $newSelling
                                ->pluck('s_i_tax')
                                ->unique()
                                ->toArray();
                        ...
                        <tr class="border-top border-bottom">
                            ...
                            <td colspan="2" class="text-end  my-bdr-right ">{{ sprintf('%.2f', $totalqty) }}
                            </td>
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td width="12.28%">
                                    {{ sprintf('%.2f', ($rootprice * $item['item_gst_slab']) / 100 / 2) }}</td>
                                <td width="12.28%">
                                    {{ sprintf('%.2f', ($rootprice * $item['item_gst_slab']) / 100 / 2) }}</td>
                                <td width="12.28%">{{ sprintf('%.2f', ($rootprice * $item['item_gst_slab']) / 100) }}
                                </td>
                                @php
                        ...
                        <tr class="border-top border-bottom ">
                            ...
                            <td class="border-right-left" width="12.28%">{{ sprintf('%.2f', $totalTaxable) }}
                            </td>
