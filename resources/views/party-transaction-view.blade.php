                    ...
                    <div class="card">
                        <form action="{{url('print-party-statement')}}" method="post" id="filter-form">
                            ...
                            <div class="row border-bottom p-3 d-flex align-items-center">
                                <div class="col-lg-6">
                                    <h2 class="m-0">{{$party->p_name}}</h2>
                                </div>
                                <div class="col-lg-6">
                                    <div class="row d-flex align-items-center">
                                        <div class="col-lg-3">
                                            <div class="form-floating">
                                                <input type="hidden" name="p_id" id="p_id" value="{{$party->p_id}}">
                                ...
                                <tbody id="transactions-list">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-success">{{sprintf('%.2f',$transaction->tnx_p_amount)}}</div>
                                            @endif
                                        </td>
                                        <td>
                                            @if ($transaction->tnx_type == 2)
                                            <div class="text-danger">
                                                @if ($transaction->tnx_p_amount != 0)
                                                {{sprintf('%.2f',$transaction->tnx_p_amount)}}
                                                @else
                                                {{sprintf('%.2f',$transaction->tnx_amount)}}
                                                @endif
                                            </div>
                                            @endif
                                        </td>
                                        <td>
                                            {{sprintf('%.2f',$transaction->tnx_final_dues)}}
                                        </td>
                                        <td>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#share_wp', function() {
            var url = 'https://api.whatsapp.com/send?phone=919771999460&text=hi';
            let from_date = $('#from_date').val();
            let to_date = $('#to_date').val();
            let p_id = $('#p_id').val();
            $.post("{{url('generate-user-statement-wplink')}}", {
                ...
                if (res.transactions.length == 0){
                    ...
                    wadata = `Dear *${res.user.p_name}*, Your Statements are:%0D%0ADate: *From* ${sdate} *to* ${edate}%0D%0APresent Dues: Rs ${res.user.p_dues}/-%0D%0A*No. of Transactions* ${res.transactions.length}:%0D%0A`;
                ...
                url = `https://api.whatsapp.com/send?phone=91${res.user.p_fmob}&text=${wadata}`;
